...
void DesignExtractor::PopulatePKB(std::shared_ptr<ProgramNode> root) {
    ...
    UsesExtractor uses_extractor(write_manager);
    ModifiesExtractor modifies_extractor(write_manager);
    ...
    root->Extract(uses_extractor);
    root->Extract(modifies_extractor);
